...
BooCell.prototype.setCharColor = function(params) {
	var params = params || {};
	if (!params.color) {
		if (lyrics.colorChars) {
			console.log("colorChars");
			var colorRed = getRandomInt(0, 255);
			var colorGreen = getRandomInt(0, 255);
			var colorBlue = getRandomInt(0, 255);
			params.color = "rgba({0}, {1}, {2}, 1)".format(colorRed, colorGreen, colorBlue);
		} else {
...
BooCell.prototype.setBackgroundImage = function(fileNames, self) {
	...
	if (!playing) {
		self.imageTimeout = window.setTimeout(function() { self.setBackgroundImage(fileNames, self) }, 500);
		return;
	}
